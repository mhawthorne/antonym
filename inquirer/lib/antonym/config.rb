module Antonym
  ...
  class Configuration
    ...
    def self.from_stream(stream)
      ...
      Configuration.new(app_name, URI.parse(raw_url), user, passwd, admin)
    end
    
    attr_reader :app_name, :app_uri, :user, :password, :admin
    
    def initialize(app_name, app_uri, user, password, admin)
      @app_name = app_name
      @app_uri = app_uri
      @user = user
      @password = password
