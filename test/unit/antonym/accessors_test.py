...
class ArtifactAccessorTest(unittest.TestCase):
    ...
    def test_save_duplicate(self):
        ...
        self.moxer.ReplayAll()
        try:
            ArtifactAccessor.save(**self.__keywords())
            self.fail("exception expected")
    ...
    def test_save(self):
        ...
        self.moxer.StubOutWithMock(ArtifactInfo, "save", use_mock_anything=True)
        self.moxer.StubOutWithMock(ArtifactContent, "save", use_mock_anything=True)
        
        source = MockEntity(MockKey(name=source_name))
        ...
        ArtifactInfo.save(**info_save_kw).AndReturn(info_key)
        
        content_save_kw = dict(source=source, source_name=source_name, info=info_key, body=body)
        ArtifactContent.save(info_key.name(), **content_save_kw).AndReturn(MockKey(name=self.test_id))
        
        self.moxer.ReplayAll()
        info, content, source = ArtifactAccessor.save(**accessor_save_kw)
